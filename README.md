...
1. Lists all secrets under a given path via `list-items` (with auto-pagination)
2. Checks each secret for an existing sync association via `describe-item`
3. Creates new sync associations or re-syncs existing ones via `static-secret-sync` / `rotated-secret-sync`
4. Optionally detects value drift between Akeyless and the remote secret store
...
The script discovers available folders and USCs, then prompts you to select them.

### CLI Arguments

```
# Sync all static secrets under a folder
./sync-folder-to-usc.py \
...
# Check for drift between Akeyless and the remote
./sync-folder-to-usc.py \
  --folder /team/app/prod \
  --usc /9-USC/AWS-TEST-Folder-Sync \
  --check-drift
...
# Quiet mode for cron/CI — suppress progress, keep results
./sync-folder-to-usc.py \
  --folder /team/app/prod \
  --usc /9-USC/AWS-TEST-Folder-Sync \
  --quiet
...
./sync-folder-to-usc.py --config sync.conf
```
...
# Secret types to sync (comma-separated: static-secret, rotated-secret)
TYPES=static-secret
...
| `--quiet` | Suppress progress messages (banner, listing); keep status lines and summary | `false` |
...
| `AKEYLESS READ FAILED` | Could not read the Akeyless value |
| `REMOTE READ FAILED` | Could not read the remote value |
| `REMOTE DECODE FAILED` | Remote value could not be base64-decoded |
...
To **fix drift**, re-run the script without `--check-drift`. The re-sync pushes Akeyless values back to the remote, overwriting any direct changes. Akeyless is always the source of truth.
...
| Code | Meaning |
|---|---|
| `0` | All operations succeeded / no drift or errors |
| `1` | Invalid arguments or configuration error |
| `N > 0` | Number of failures (sync) or drifted + errored secrets (drift), capped at 125 |
...
- `RE-SYNC` — existing association found, value re-pushed to remote
...
## Underlying Akeyless CLI Commands

| Command | Purpose |
|---|---|
| `akeyless list-items --path <folder> --type <type> --auto-pagination enabled` | List all secrets under a folder |
...
| `akeyless get-secret-value --name <secret>` | Read Akeyless value (drift check) |
| `akeyless usc get --usc-name <usc> --secret-id <arn>` | Read remote value (drift check) |
...
```
# Cron — sync every hour (quiet mode for clean logs)
0 * * * * /path/to/sync-folder-to-usc.py --config /path/to/sync.conf --quiet

# Cron — drift check daily, alert on failure
0 6 * * * /path/to/sync-folder-to-usc.py --config /path/to/drift.conf --check-drift --quiet || notify-team.sh
```
